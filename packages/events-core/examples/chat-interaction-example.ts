...
async function setupTestTaskAndSubtask(taskRepo: TaskRepository): Promise<[Task, Subtask]> {
  const taskId = uuidv4();
...
function setupCustomEventHandlers(eventBus: ReturnType<typeof createServerEventBus>, chatId: string) {
...
async function runChatInteractionExample(): Promise<void> {
  ...
  if (chat) {
    logger.info(`Chat retrieved successfully. It contains ${chat.messages.length} messages.`);
    // Log message content for demonstration
    chat.messages.forEach((message, index) => {
      logger.info(`Message ${index + 1}: [${message.role}] ${message.content}`);
    });
  } else {
    logger.warn("Chat could not be retrieved. It might not have been properly saved.");
  }
  ...
  logger.info("4. CLIENT_SUBMIT_INITIAL_PROMPT_COMMAND - Submit initial prompt");
  ...
  logger.info("7. SERVER_AGENT_PROCESSED_MESSAGE - Agent processes the message");
  logger.info("8. SERVER_AGENT_RESPONSE_GENERATED - Agent generates a response");
